...
public class Round393B {
	...
	void solve() throws IOException {
		...
		while (left <= right) {
			...
			long minimumX;
			if (x >= mid - 1) {
				minimumX = (1 + mid - 1) * (mid - 1) / 2 + (x - (mid - 1));
			} else {
				minimumX = mid * x - (x + 1) * x / 2;
			}

			long minimumY;
			if (y >= mid - 1) {
				minimumY = (1 + mid - 1) * (mid - 1) / 2 + (y - (mid - 1));
			} else {
				minimumY = mid * y - (y + 1) * y / 2;
			}

			if (m - (mid + minimumX + minimumY) >= 0) {
				ans = mid;
				left = mid + 1;
			} else {
				right = mid - 1;
